...
# FIX 1: Do NOT divide by 255 manually — the model handles preprocessing internally
# (MobileNetV2's preprocess_input is baked into the model)
...
# FIX 2: class_names[pred_index] gives the folder name (e.g., "1"), look it up in cat_to_name
pred_label = class_names[pred_index]
pred_name  = cat_to_name.get(str(pred_label), "Unknown")   # ensure string key
...
print("Number of classes in class_names.json:", len(class_names))
print(f"Predicted index: {pred_index} → label: '{pred_label}' → name: '{pred_name}'")
